...
class SupportSystem():
    ...
    def kb_retriever(self) -> Agent:
        return Agent(
            config=self.agents_config['kb_retriever'], # type: ignore[index]
            verbose=True
        )
    ...
    def solution_generator(self) -> Agent:
        return Agent(
            config=self.agents_config['solution_generator'], # type: ignore[index]
            verbose=True
        )
    ...
    def ticket_classification_task(self) -> Task:
        return Task(
            config=self.tasks_config['ticket_classification_task'], # type: ignore[index]
        )

    @task
    def kb_retrieval_task(self) -> Task:
        return Task(
            config=self.tasks_config['kb_retrieval_task'], # type: ignore[index]
        )

    @task
    def solution_generation_task(self) -> Task:
        return Task(
            config=self.tasks_config['solution_generation_task'], # type: ignore[index]
        )
